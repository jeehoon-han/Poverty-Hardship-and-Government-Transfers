...
* 1) SNAP
* step1. use probit model to predict the likelihood of SNAP receipt

bysort year serial famid: gen fsize = _N
replace fsize=14 if fsize>=15
bysort year serial famid: egen fnchild = sum(age<=17)
bysort year serial famid: egen fwchild = max(age<=17)
replace fnchild=9 if fnchild>=9
bysort year serial famid: egen fweld = max(age>=60)
bysort year serial famid: egen fwdis = max(incdisab>0)
bysort year serial famid: egen fwwelfinc = max(incwelfr>0)
gen nrace = 0
replace nrace = 1 if race==100 // white 
replace nrace = 2 if race==200 // black
replace nrace = 3 if race>=300 // other
gen nhisp = hispan!=0
gen nedu = 0
replace nedu = 1 if educ<73  // less than HS
replace nedu = 2 if educ==73 // HS degree 
replace nedu = 3 if educ==81 // Some college but no degree
replace nedu = 4 if educ==91|educ==92 // Associate degree
replace nedu = 5 if educ==111 // Bachelor's degree
replace nedu = 6 if educ>=123 // Master's degree or more

gen FPL=0
replace FPL = 11770+4160*(fsize-1) if ref_year==2015
replace FPL = 11880+4140*(fsize-1) if ref_year==2016
replace FPL = 12060+4180*(fsize-1) if ref_year==2017
replace FPL = 12140+4320*(fsize-1) if ref_year==2018
replace FPL = 12490+4420*(fsize-1) if ref_year==2019
replace FPL = 12760+4480*(fsize-1) if ref_year==2020
replace FPL = 12880+4540*(fsize-1) if ref_year==2021
replace FPL = 13590+4720*(fsize-1) if ref_year==2022
replace FPL = FPL*1.25 if statefip==2
replace FPL = FPL*1.15 if statefip==15

/*
preserve 
keep if firstper==1

gen fsrcpt = foodstamp>0
gen fsval = foodstamp
replace fsval=. if foodstamp==0

gen inc_FPL = ftotval/FPL
replace inc_FPL=0 if inc_FPL<0
keep if inc_FPL<2.5
probit fsrcpt inc_FPL, allbase   

probit fsrcpt inc_FPL i.fsize i.fwchild fweld fwdis fwwelfinc i.sex i.nrace i.marst i.nhisp i.nedu i.statefip i.year [pw=asecwth], allbase  
predict prob, pr 


* step2. Impute the benefit amount using Predictive mean matching (PMM) method 
mi set wide
mi register imputed fsval
mi impute pmm fsval inc_FPL i.fsize i.fwchild fweld fwdis fwwelfinc i.sex i.nrace i.marst i.nhisp i.nedu i.statefip i.year, add(1) knn(1) rseed(82)

order year serial famid fsrcpt prob fsval foodstamp _mi_miss _1_fsval 
replace fsval=_1_fsval
forvalues i = 1/15 {
bysort year: gen rn`i' = runiform(0,1)
}
gen round1 = prob>rn1
forvalues i = 1/14 {
local j = `i'+1
gen round`j' = round`i'==1|prob>rn`j'	
}

* create a random number for ordering within round
bysort year: gen rn = floor(runiform()*_N) + 1


keep if _mi_miss==1
keep year serial famid fsval prob round* rn
save imp_snap, replace
restore
*/
...
* test aggregate 
/*
replace snapval = 0 if relate!=101
bysort ref_year: egen agg_snap_imp = sum(snapval*asecwt/1000000000)
collapse agg_snap_imp, by(ref_year)
list 
*/
...
bysort year serial fam_type: egen tot_inclessUI = sum(inctot-incunemp)   // pre-tax income w/o UI
...
********************************************************************************
** imputed UI by income quartile 
preserve
replace tot_inclessUI = tot_inclessUI/scale 
bysort ref_year: egen quint_inc = xtile(tot_inclessUI), nq(5) weight(asecwt)
gen agg_UI_byq = UI*asecwt/1000000000
gen agg_ui_byq = incunemp*asecwt/1000000000
collapse (sum) agg_UI_byq agg_ui_byq, by(ref_year quint_inc)
list
restore
********************************************************************************
...
* after-tax income - noncash benefits 
*gen inc3a = inc2a - (tot_snap + tot_lunch + tot_energy + tot_wic + hous_sub)
...
foreach var in stampval snapval hous_sub snap lunch energy wic spmwic {
	replace `var' = 0 if relate!=101
}
bysort ref_year: egen agg_snap_rep = sum(stampval*asecwt/1000000000)
bysort ref_year: egen agg_snap_imp = sum(snapval*asecwt/1000000000)
...
export excel using inc_pov_v1.xls, firstrow(var) keepcellfmt replace
restore
